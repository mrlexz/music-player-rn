...
const Wheel: FC<WheelProps> = ({listData, textWinner}) => {
  const numOfSegments =
    listData && listData.length > 0 ? listData.length : numOfSegmentsDefault;
  ...
  const _wheelPaths = React.useMemo(() => {
    const data = Array.from({
      length: listData && listData.length > 0 ? listData.length : numOfSegments,
    }).fill(1) as Array<number>;
    ...
    return arcs.map((arc, index) => {
      ...
      return {
        path: instance(arc),
        color: colors[index],
        value: listData && listData.length > 0 ? listData[index] : `${index}`,
        centroid: instance.centroid(arc),
      };
    });
  }, [listData, numOfSegments]);
